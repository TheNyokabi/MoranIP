...
export interface Account {
    ...
    balance?: number // Derived in frontend or added to API later
}
...
export const accountingApi = {
    getAccounts: (token: string): Promise<Account[]> =>
        erpNextApi.listResource(token, 'Account'),

    getAccount: (token: string, name: string): Promise<Account> =>
        erpNextApi.getResource(token, 'Account', name),

    getGLEntries: (token: string, filters?: any): Promise<GLEntry[]> =>
        erpNextApi.listResource(token, 'GL Entry', filters),
}
