...
CODE_PATTERN = re.compile(r"^LID-[A-Z]{2}-\d{2}-[0-9A-Z]{5}$")
...
def main() -> int:
    ...
    base = f"/api/tenants/{tenant_id}/erp/crm/leads"

    create_body = {
        "lead_name": f"Test Lead {time.strftime('%H%M%S')}-{suffix}",
        "email_id": email,
        "status": "Open",
    }
    ...
    if not CODE_PATTERN.match(created_code):
        print("ERROR: Lead ID is not human-readable code format")
