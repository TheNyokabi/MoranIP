...
CODE_PATTERN = re.compile(r"^CON-[A-Z]{2}-\d{2}-[0-9A-Z]{5}$")
...
def main() -> int:
    ...
    email = f"test.contact.{suffix}@example.com"

    base = f"/api/tenants/{tenant_id}/erp/crm/contacts"

    create_body = {
        "first_name": "Test",
        "last_name": f"Contact {time.strftime('%H%M%S')}-{suffix}",
        "email_id": email,
    ...
    if not CODE_PATTERN.match(created_code):
        print("ERROR: Contact ID is not human-readable code format")
